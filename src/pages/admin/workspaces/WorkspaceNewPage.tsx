    ...
    <div className="w-full">
      <PageHeader
        breadcrumbs={[
          ...
          { label: language === 'vi' ? 'Tạo mới' : 'Create New' },
        ]}
        title={language === 'vi' ? 'Tạo Workspace mới' : 'Create New Workspace'}
        ...
        <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border-color)] shadow-sm overflow-hidden">
          <div className="p-5 border-b border-[var(--border-color)] flex items-center gap-3">
            ...
            <h3 className="text-sm font-bold text-[var(--text-primary)]">
              {language === 'vi' ? 'Thông tin Workspace' : 'Workspace Information'}
            </h3>
          </div>
          ...
          <div className="p-6 space-y-6">
                  ...
                  <button
                    ...
                    className={`relative aspect-square overflow-hidden rounded-lg border transition-all ${
                      avatar === url
                        ...
                        : 'border-[var(--border-color)] hover:border-sky-400/60'
                    }`}
                  >
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { value: 'kanban', Icon: Columns, title: 'Kanban', desc: language === 'vi' ? 'Tập trung vào luồng công việc liên tục.' : 'Focus on continuous workflow.' },
                  { value: 'scrum', Icon: Repeat, title: 'Scrum', desc: language === 'vi' ? 'Làm việc theo sprint định kỳ với backlog.' : 'Work in periodic sprints with a backlog.' },
          ...
          <div className="px-6 py-4 border-t border-[var(--border-color)] bg-[var(--bg-tertiary)]/30 flex justify-end gap-3">
            ...
            <Button type="submit" disabled={isSaving} className="px-6">
              {isSaving ? (language === 'vi' ? 'Đang tạo...' : 'Creating...') : (language === 'vi' ? 'Tạo mới Workspace' : 'Create Workspace')}
            </Button>
          </div>
        </div>
      </form>
